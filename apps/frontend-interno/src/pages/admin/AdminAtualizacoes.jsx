...
function AdminAtualizacoes() {
  ...
  const carregar = async () => {
    try {
      ...
      console.error('Erro ao carregar atualizações');
    }
  };
  ...
  const filtrados = atualizacoes.filter((item) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => navigate('/admin/atualizacoes/novo')}
            ...
            {filtrados.map((item) => (
              <tr key={item.id} className="border-t hover:bg-slate-50">
                ...
                <td className="p-4 flex justify-end gap-2">
                  <button
                    onClick={() => navigate(`/admin/atualizacoes/${item.id}`)}
...
export default AdminAtualizacoes;