...
function AdminManuais() {
  ...
  const [tipoFiltro, setTipoFiltro] = useState('Manual'); // 🔥 Mantido para filtrar por categoria
  ...
  // 🔥 FILTRO: Agora busca tanto no campo TÍTULO quanto na DESCRIÇÃO
  const filtrados = manuais.filter((item) => {
  ...
  // 🔥 Highlight (agora pode ser usado tanto no título quanto na descrição)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => navigate('/admin/manuais/novo')}
            ...
            {filtrados.map((item) => (
              <tr key={item.id} className="border-t hover:bg-slate-50">
                ...
                <td className="p-4 flex justify-end gap-2">
                  <button
                    onClick={() => navigate(`/admin/manuais/${item.id}`)}
