...
function AdminRamais() {
  ...
  const carregarRamais = async () => {
    try {
      const { data } = await axios.get('/api/ramais');
      setRamais(data);
    } catch (err) {
  ...
  const excluirRamal = async (id) => {
    ...
    try {
      await axios.delete(`/api/ramais/${id}`);
      carregarRamais();
    } catch (err) {
          ...
          <tbody>
            {ramaisFiltrados.map((ramal) => (
              <tr key={ramal.id} className="border-t hover:bg-slate-50">
                <td className="p-4 font-medium capitalize">{highlight(ramal.setor)}</td>
                <td className="p-4 capitalize">{highlight(ramal.subsetor)}</td>
                <td className="p-4 font-mono font-bold capitalize">{highlight(ramal.numero.toString())}</td>
